...
if ( ! function_exists('generatehtml'))
{
    ...
    function textareaW($name,$id,$class,$rows,$values,$classw)
    {
            return "<div class='$class'><textarea name='".$name."' id='".$id."' rows='".$rows."' class='form-control '".$classw."' '>".$values."</textarea></div>";
    }
    ...
    function editcombo($nama,$table,$class,$field,$pk,$kondisi,$tags,$value)
    {
     ...
     function editcombokhu($nama,$table,$class,$field,$pk,$kondisi,$tags,$value)
    {
        $CI =& get_instance();
        $CI->load->model('mcrud');
        if(empty($tags))
        ...
        foreach ($data as $r)
        {
            echo"<option value='".$r->$pk."' ";
	    echo $r->$pk==$value?"selected='selected'":"";
	    echo">".strtoupper(tgl_indo($r->$field))."</option>";
        }
            echo"</select></div>";
    }
